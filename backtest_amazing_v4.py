...
import os as _os
_hf_orig_dl = _hfd.hf_hub_download
def _hf_patched_dl(repo_id, filename=None, **kwargs):
    if _os.path.isdir(repo_id):
        if filename:
            fp = _os.path.join(repo_id, filename)
            if _os.path.exists(fp):
                return fp
        fp = _os.path.join(repo_id, 'config.json')
        if _os.path.exists(fp):
            return fp
        return repo_id
    return _hf_orig_dl(repo_id, filename=filename, **kwargs)
_hfd.hf_hub_download = _hf_patched_dl

_hf_orig_val = _v.validate_repo_id
def _hf_patched_val(repo_id):
    if _os.path.isdir(repo_id):
        return
    return _hf_orig_val(repo_id)
_v.validate_repo_id = _hf_patched_val

KRONOS_BASE = os.path.expanduser('~/workspace/Kronos')
sys.path.insert(0, KRONOS_BASE)
from model import Kronos, KronosTokenizer, KronosPredictor

MODEL_PATH = os.path.join(KRONOS_BASE, 'models/Kronos-base')
...
def run_batch_kronos(stock_list, batch_size=15):
    """对列表中的股票分批执行 Kronos 预测 (避免内存溢出)"""
    valid_stocks = []
    df_list = []
    x_ts_list = []
    y_ts_list = []
    ...
    for s in stock_list:
        ...
        try:
            ...
            df_list.append(df_hist.reset_index(drop=True))
            x_ts_list.append(x_ts.reset_index(drop=True))
            y_ts_list.append(y_ts.reset_index(drop=True))
    ...
    for batch_start in range(0, len(valid_stocks), batch_size):
        ...
        batch_df = df_list[batch_start:batch_end]
        batch_x = x_ts_list[batch_start:batch_end]
        batch_y = y_ts_list[batch_start:batch_end]

        try:
            pred_dfs = predictor.predict_batch(
                batch_df, batch_x, batch_y,
                pred_len=5, T=1.0, top_p=0.9, sample_count=10, verbose=False
            )
            for i, pred_df in enumerate(pred_dfs):
                if pred_df is not None and len(pred_df) > 0:
                    stock = valid_stocks[batch_start + i]
                    last_close = float(stock['daily_df']['close'].iloc[-1])
                    avg_pred = pred_df['close'].head(2).mean()
                    kronos_pred = (avg_pred - last_close) / last_close * 100
                    stock['kronos_pred'] = kronos_pred
        except Exception as e:
            print(f'    batch [{batch_start}-{batch_end}] Kronos 预测失败: {e}')
...
def run_backtest(trading_dates):
    ...
    for day_idx, trade_date in enumerate(trading_dates):
        ...
        # 阶段2b: 预计算 tail_score, 过滤出候选 (减少 Kronos 调用)
        pre_scored = []
        ...
        # 仅对前 20 只运行 Kronos (其余 kronos_pred = None)
        kronos_top = [r for _, r in pre_scored[:20]]
        print(f'  技术分析完成: {len(technicals_list)} 只, 对 top {len(kronos_top)} 进行 Kronos 预测...')
        run_batch_kronos(kronos_top)
